    ...
    $.fn.dragTable = function (options) {
        ...
        $table.find("tr:first th").mousemove(function (event) {
            ...
            if (null != $th && $th.mouseDown == true) {
                if (event.pageX - $th.oldX > 0) {
                    $th.width(event.pageX - $th.oldX);
                    var id = $table.attr("id");
                    $("#" + id + "_headBox").html('<table class="table table-bordered table-condensed"><thead>' + $table.find("thead").html() + "</thead></table>");
                }
            }
        });
