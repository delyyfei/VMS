...
Alert.prototype = {
    ...
    init: function (options) {
        ...
        if (this.$alertDialog.length == 0) {
            $("body").append('<div class="modal" id="alertDialog" tabindex="-1"><div class="modal-dialog" style="width:300px;"><div class="modal-content"><div class="modal-header"></div><div class="modal-body"></div><div class="modal-footer" style="text-align:center;"><button class="btn btn-default"><span class="glyphicon glyphicon-remove"></span> 取消</button><button class="btn btn-primary"><span class="glyphicon glyphicon-ok"></span> 确认</button></div></div></div></div>');
        }
        this.$alertDialog = $(this.options.alertDiv);
        this.$alertHeader = this.$alertDialog.find(".modal-header");
    ...
    confirm: function () {
        if (this.actionFlag && typeof this.options.confirmAction === "function") {
            if (this.options.needConfirm && this.options.needValidate) {
                var formObj = $(this.options.validateForm).inputValid();
                if (Boolean(formObj)) {
